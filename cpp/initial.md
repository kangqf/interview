...
T array[N] = {other};	//作为聚合初始化的一部分，初始化提供了初始化器的每个元素	
```

#### 初始化规则

* 首先，若 T 为**类类型** 且 初始化器为**纯右值**表达式 ，而其**无 cv 限定**类型是**与 T 相同**的类，就以初始化器表达式自身初始化目标对象（也就是移动构造函数）
...
* 作为 1 `非类类型 的 值初始化序列`和 2 `无默认构造函数的类 类型成员 的 值初始化序列` 的一部分 以及 3  `未提供初始化器的聚合体元素 的 值初始化`
...
[参考](http://blog.csdn.net/u014359097/article/details/50788911)